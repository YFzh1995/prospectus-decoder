...
🔧 深度：Level [X] | 开始处理...
```
6. **版本检查**（可选、低开销）：若网络可用，尝试 `webfetch` 远程 `VERSION` 文件（地址：`https://raw.githubusercontent.com/YFzh1995/prospectus-decoder/main/VERSION`）。若返回的远程版本号 > 本地 `VERSION` 文件中的版本号，则在确认信息后追加一行提示：
```
🆕 检测到新版本 v[X.X.X]（当前：v[Y.Y.Y]）。建议更新：git pull 或重新运行安装脚本。
```
若网络不可用或版本相同，静默跳过。
